// In-browser "2D → 3D relief asset" tool.
//
...
export class ReliefAssetTool {
  constructor({ onOpen, onClose, onCreateAsset } = {}) {
    this.onOpen = onOpen;
    this.onClose = onClose;
    this.onCreateAsset = onCreateAsset;
    this.isOpen = false;
  ...
  _buildDOM() {
    ...
    panel.innerHTML = `
      <div style="font-size:13px;letter-spacing:.14em;color:#7fdca0">CREATE RELIEF ASSET</div>
      <div style="color:#8fa899;font-size:11px">Draw or load a photo, then add the relief as a placeable world asset.</div>
    `;
    ...
    btnRow.appendChild(this._button("Add Asset", () => this._createAsset()));
  ...
  _generate() {
    ...
    const plane = this.createGeometry();
