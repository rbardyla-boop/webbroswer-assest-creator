...
export class PlayerController {
  constructor(player, input, cameraController, colliderSystem) {
    this.player = player;
    this.input = input;
    this.cam = cameraController;
    this.colliders = colliderSystem;
    ...
    p.y = this._groundHeight(p.x, p.z);
  ...
  update(dt) {
    ...
    this.colliders?.resolveHorizontalCapsule(player.position, previous, player.radius, player.height);

    // Vertical: jump + gravity, clamped to the terrain.
    const groundY = this._groundHeight(player.position.x, player.position.z, player.position.y);
  ...
  _groundHeight(x, z, playerY = Infinity) {
    return this.colliders?.getSupportHeight(x, z, playerY) ?? getHeight(x, z);
  }
}
